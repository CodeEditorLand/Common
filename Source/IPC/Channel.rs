...
DefineChannels! {
	...
	AuthValidateToken                             => "auth:validateToken",
		...
		// `UserInterface.Show*Dialog` — dotted names mirrored from Cocoon→Mountain
		// gRPC; the Wind-side Files/Live.ts routes them through Tauri IPC today.
		// Both sides accept the dotted form.
		//
		// `mountain_get_status` — snake_case Tauri command.
