...
impl TransportError {
	...
	pub fn IsRetryable(&self) -> bool { self.Code.IsRetryable() }

	/// Returns the recommended retry delay in milliseconds.
	pub fn RetryDelayMilliseconds(&self) -> u64 { self.Code.RecommendedRetryDelayMilliseconds() }
...
impl fmt::Display for TransportError {
	fn fmt(&self, Formatter:&mut fmt::Formatter<'_>) -> fmt::Result { write!(Formatter, "{}", self.FullMessage()) }
}
...
impl TransportError {
	...
	pub fn InvalidRequest(Message:impl Into<String>) -> Self { Self::New(TransportErrorCode::InvalidRequest, Message) }

	/// Not supported error: feature not implemented by this transport.
	pub fn NotSupported(Message:impl Into<String>) -> Self { Self::New(TransportErrorCode::NotSupported, Message) }

	/// Remote error: the remote endpoint returned an error.
	pub fn Remote(Message:impl Into<String>) -> Self { Self::New(TransportErrorCode::RemoteError, Message) }

	/// Internal error: something went wrong inside the transport.
	pub fn Internal(Message:impl Into<String>) -> Self { Self::New(TransportErrorCode::InternalError, Message) }

	/// Circuit breaker open error: request rejected due to circuit breaker.
	pub fn CircuitBreakerOpen() -> Self {
		Self::New(TransportErrorCode::CircuitBreakerOpen, "Circuit breaker is open").WithTransportKind("unknown")
	}
	...
	pub fn NotFound(Message:impl Into<String>) -> Self { Self::New(TransportErrorCode::NotFound, Message) }
