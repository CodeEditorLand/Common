...
/// @module OutputChannelManager
/// @description Defines the abstract service trait for managing output
/// channels.
...
pub trait OutputChannelManager: Environment + Send + Sync {
	...
	///
	/// @param Name - The human-readable name of the channel to be displayed in
	/// the UI. @param LanguageIdentifier - An optional language ID to enable
	/// syntax highlighting   for the channel's content.
	/// @returns A `Result` containing a unique identifier (string) for the new
	/// channel.
	...
	///
	/// @param ChannelIdentifier - The unique ID of the target channel.
	/// @param Value - The string content to append.
	...
	///
	/// @param ChannelIdentifier - The unique ID of the target channel.
	/// @param Value - The new string content for the channel.
	...
	///
	/// @param PreserveFocus - If `true`, the focus will remain in its current
